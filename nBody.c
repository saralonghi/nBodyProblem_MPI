...
int main(int argc, char **argv) {
    ...
    if (rank == MASTER) {    printBodies(bodies, numberOfBodies, numberOfProc, iterations,1); printExecutionTime(bodies, numberOfBodies, numberOfProc, iterations,executionTime);}
...
void printCmd() { printf("To correctly launch nBody run: mpirun -np <Processors> nBody <Bodies> <Iterations> [-R|-G]\n"); }
